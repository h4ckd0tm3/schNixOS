...
# media-control is a wrapper that execs this perl adapter, so that is the name to
# kill. Older instances from previous reloads would otherwise keep streaming and
# race this one on the artwork file.
...
/opt/homebrew/bin/media-control stream --no-diff --debounce=250 | while IFS= read -r line; do
  ...
  printf '%s' "$line" | jq -r '.payload.artworkData // empty' | base64 -d > "$TMP" 2>/dev/null
  if [ -s "$TMP" ] && sips -Z 64 "$TMP" >/dev/null 2>&1; then mv -f "$TMP" "$ART"; else rm -f "$ART" "$TMP"; fi
